...
class BattleTextParser {
	...
	static upgradeArgs({args, kwArgs}: {args: Args, kwArgs: KWArgs}): {args: Args, kwArgs: KWArgs} {
		...
		case '-activate': {
			...
			if ([
				...
				'electricterrain', 'mistyterrain', 'psychicterrain', 'hologram', 'telepathy', 'stickyhold', 'suctioncups', 'aromaveil',
		...
		case 'cant': {
			...
			if (['ability: Damp', 'ability: Dazzling', 'ability: Queenly Majesty', 'ability: Armor Tail', 'ability: Yandere', 'ability: Rain Shaman', 'ability: Dancing Siren', 'ability: YabaIRyS'].includes(effect)) {
				args[0] = '-block';
	...
	parseArgsInner(args: Args, kwArgs: KWArgs) {
		...
		case '-activate': {
			...
			if (!target &&
				['hyperdrill', 'hyperspacefury', 'hyperspacehole', 'phantomforce', 'shadowforce', 'fisted', 'macesmash', 'feint'].includes(id)) {
