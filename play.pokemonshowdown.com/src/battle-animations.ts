...
export class BattleScene implements BattleSceneStub {
	...
	addSideCondition(siden: number, id: ID, instant?: boolean) {
		...
		case 'hologram':
			...
			break;
		case 'infirmary':
			const infirmary = new Sprite(BattleEffects.heart, {
				display: 'block',
				x,
				y,
				z: side.behind(-27),
		...
		case 'infirmary2':
			...
			this.$spritesFront[spriteIndex].append(heart1.$el!);
			this.$spritesFront[spriteIndex].append(heart2.$el!);
			this.$spritesFront[spriteIndex].append(heart3.$el!);
			this.$spritesFront[spriteIndex].append(heart4.$el!);
			this.sideConditions[siden][id] = [heart1, heart2, heart3, heart4];
			break;		
		case 'gmaxsteelsurge':
